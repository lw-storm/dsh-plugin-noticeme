...
export function apply(ctx) {
  ctx.on('approval/request', async (req, next) => {
    ...
    try {
      push({ ...summarizeApproval(req), callId })
    } catch {
      /* never block approval flow */
    }
    // Transparent waterfall: wait for the decision so we can mark the call
    // settled the MOMENT the user approves/rejects — not when the underlying
    // tool finishes. Otherwise "approve, then switch away" re-notifies.
    let outcome
    try {
      outcome = await next()
    } catch {
    ...
    if (callId && (outcome === 'allowed-once' || outcome === 'rejected' || outcome === 'cancelled')) {
      markResolved(callId)
    }
    return outcome
  })
